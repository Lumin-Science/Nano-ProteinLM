...
top-L rule, and chain-mean reduction. Never edit the evaluator or its data.
...
- four GPUs;
- 7,200 seconds of synchronized training-loop wall time.
...
outside the 7,200-second training clock, but their durations must be logged.
...
5. Run the unchanged code first to establish the two-hour P@L baseline.

Do not propose a candidate before the baseline succeeds.

## Estimate the two-hour iteration budget
...
4. Compute:
   ...
   estimated_steps = ceil(1.02 * 7200 / seconds_per_step)
   ```

5. Set the actual run's step target to `estimated_steps`. Step-based warmup or
   decay must use this estimate.
6. Keep `walltime_seconds: 7200` as the authoritative hard stop.
...
step target materially before two hours, correct the estimate and rerun. Log
...
6. Smoke-test it and calculate its two-hour step estimate.
7. Train from scratch on four GPUs for the synchronized 7,200-second budget.
...
improvements. Never weaken the P@L evaluator or the four-GPU/two-hour contract.
