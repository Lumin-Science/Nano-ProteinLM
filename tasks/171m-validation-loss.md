...
- **Score:** held-out sequence-mean MLM negative log-likelihood, **lower is better**. Read `validation_mlm.sequence_mean_nll` from the run's `evaluation/EVALUATION.json`. Search runs do not evaluate contact P@L.
...
- **Checkpoint:** score the final checkpoint at the training-time limit. A run must complete training and the MLM evaluation to supply a task score. Report the actual training duration and any overrun from the final optimizer update.
...
Prepare enough of the provided corpus for each final recipe's selected source mixture and token budget. Retain the same tokenizer, context, parameter-size and evaluation boundaries. The owner scores each final checkpoint with the same full MLM evaluation and also reports contact P@L over all 20,775 chains, with its 5,000-replicate chain-bootstrap 95% interval. One training seed does not estimate training-seed variability. Because search uses these same evaluation assets, this test measures transfer to longer training rather than performance on a blind holdout.
