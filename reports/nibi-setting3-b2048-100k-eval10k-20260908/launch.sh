#!/usr/bin/env bash
set -euo pipefail
mode=${1:?trial, resume4 or full}
case "$mode" in trial|resume4|full) ;; *) exit 2 ;; esac
...
durable=/project/def-lsigal/muchenli/Nano-Protein-LM/checkpoints/nibi-setting3-b2048-100k-eval10k-20260908
...
world=8
if [[ "$mode" == resume4 ]]; then world=4; export CUDA_VISIBLE_DEVICES=0,1,2,3; fi
...
config=yaml.safe_load((repo/'configs/esmc-171m-setting3-nibi-fa3-b2048-stage1-100k.yaml').read_text())
if mode in ('trial','resume4'):
    config.update(max_steps=20,schedule_steps=20,warmup_steps=5,log_interval=1,checkpoint_interval=10,periodic_evaluation_interval=10,walltime_seconds=1800)
    if mode=='resume4':
        config.update(max_steps=22,schedule_steps=22,expected_world_size=4,periodic_evaluation_interval=0)
        config['stages'][0]['gradient_accumulation']=8
else:
    gate=json.loads((root/'QUALIFICATION_PASSED.json').read_text())
...
resume_args=()
if [[ "$mode" == resume4 ]]; then resume_args=(--resume "$root/trial/checkpoint-final.pt"); fi
...
PY
elif [[ "$mode" == resume4 ]]; then
  "$py" - "$root" "$out" "$expected_commit" <<'PY'
import json,sys
from pathlib import Path
root,out=map(Path,sys.argv[1:3])
trial=json.loads((root/'PERIODIC_EVALUATION_TRIAL_PASSED.json').read_text())
resume=json.loads((out/'RESUME.json').read_text())
verified=json.loads((out/'TRAINING_VERIFIED.json').read_text())
assert trial['status']==verified['status']=='passed'
assert trial['source_commit']==sys.argv[3]
assert verified['optimizer_steps']==22 and verified['world_size']==4
assert resume['optimizer_restored'] and resume['global_batch_preserved']
assert verified['model_and_optimizer_roundtrip_exact']
(root/'QUALIFICATION_PASSED.json').write_text(json.dumps(dict(status='passed',source_commit=sys.argv[3],periodic_evaluation_trial=trial,eight_to_four_resume=verified,execution=trial['execution']),indent=2)+'\n')
PY
else
  mkdir -p "$durable"
...
PY
  ...
  cp "$report/README.md" "$durable/README-setting3.md"
