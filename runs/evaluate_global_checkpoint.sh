...
CUDA_VISIBLE_DEVICES="${gpu_ids[0]}" "$py" -m nanoprotein.evaluate --checkpoint "$checkpoint" --data-root "$DATA_ROOT" \
  --output-root "$out/eval-validation" --validation-context 512 > "$out/validation.log" 2>&1
...
assert v['sequences']==12288 and v['masked_residues']>0 and math.isfinite(v['sequence_mean_nll'])
assert v['settings']==dict(population='all-validation',context_length=512,mask_seed=20260821)
...
if [[ -n "${DURABLE_ROOT:-}" && "${PRESERVE_MILESTONES:-0}" == 1 ]]; then
  "$py" -m nanoprotein.preserve_checkpoint --checkpoint "$checkpoint" --receipt "$out/RESULT_VERIFIED.json" --destination "$DURABLE_ROOT" --milestone-interval 100000
fi
