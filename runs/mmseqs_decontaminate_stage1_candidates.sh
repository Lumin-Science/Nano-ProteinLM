...
output_root="${OUTPUT_ROOT:-/home/muchenli/datasets/evaluation-decontamination/stage1-candidates-v1}"
...
threads="${MMSEQS_THREADS:-128}"
maximum_sequences="${MMSEQS_MAX_SEQS:-1000000}"
train_candidates="${TRAIN_CANDIDATES:-5000000}"
validation_candidates="${VALIDATION_CANDIDATES:-32768}"
...
UV_CACHE_DIR="${UV_CACHE_DIR:-/home/muchenli/.cache/uv}" \
  "$uv_bin" run --frozen python scripts/build_stage1_screen_candidates.py \
  ...
  > "$output_root/candidates.stdout" 2> "$output_root/candidates.stderr"
...
for source in uniref90 mgnify omg_img; do
  fasta="$candidate_root/$source.fasta"
...
jq -n \
  ...
  --arg training_reservoir_scope "complete eligible prefixes covering the prepared Stage-1 corpus" \
  --arg query_fasta_sha256 "$(sha256sum "$query_fasta" | awk '{print $1}')" \
  --arg ledger_sha256 "$(sha256sum "$ledger" | awk '{print $1}')" \
  --arg candidate_bundle_sha256 "$(sha256sum "$candidate_root/CANDIDATE_BUNDLE.json" | awk '{print $1}')" \
  ...
  '{
    ...
    training_reservoir_scope: $training_reservoir_scope,
...
UV_CACHE_DIR="${UV_CACHE_DIR:-/home/muchenli/.cache/uv}" \
  ...
  --source-receipt "uniref90=$candidate_root/uniref90.verification.json" \
  --source-receipt "mgnify=$candidate_root/mgnify.verification.json" \
  --source-receipt "omg_img=$candidate_root/omg_img.verification.json" \
