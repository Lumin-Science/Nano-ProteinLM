...
Each task invocation consumes one round, including a failed training attempt. The normal allowance is 72 rounds; a user-requested qualification may stop earlier. Do not launch owner-run final evaluation unless it was requested.
...
Use `baseline`, `pending` (seed 42 passed step 2), `keep`, `discard` or `failed` for the decision. `score` is the task's raw metric and `reward` its oriented value.
