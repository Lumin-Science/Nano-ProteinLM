...
Use the selected task's entry point for other objectives. Keep conditions and the common seed matched across comparisons. Read the completed run's `TRAINING_COMPLETE.json` and `evaluation/EVALUATION.json`; verify that evaluation matches the final checkpoint and includes all 12,288 validation proteins plus all 20,775 contact chains. Missing, failed or non-finite measurements cannot qualify a candidate.
