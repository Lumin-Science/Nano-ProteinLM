# AutoResearch

Read the selected task definition, defaulting to
[171M validation loss](task/171m-validation-loss.md).

Improve its score within the task boundaries and the research budget supplied
by the user. Choose your own search strategy and acceptance criteria. Use the
task's research commands to obtain comparable measurements and report the
resulting recipe with its recorded scores.

Test of Progress is run manually by the benchmark owner. Do not initiate it
unless the user explicitly requests verification.
