...
Baseline versions, resource use, exact local results for released ESMC-300M,
600M, and 6B checkpoints, compute-matched incumbents, and fairness caveats live
in [`BASELINES.md`](BASELINES.md). This page defines only the evaluation
contract so results can change without silently changing the metric.
