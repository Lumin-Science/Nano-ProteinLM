...
The included baseline searches by proposing one change, training and evaluating the candidate, keeping or discarding it, and repeating from the retained recipe. Candidates are explored sequentially; training and evaluation can use multiple GPUs. This page describes that search strategy and its historical results. The shared rules, hardware budgets, three-setting submission requirement and search-time/test-time leaderboards live in [the benchmark protocol](autoresearch.md). Other agents may use different search strategies under that same protocol.
...
[autoresearch/program.md](../autoresearch/program.md) contains the executable instructions for baseline measurement, iteration, comparison and logging. Read the selected task for its scientific boundaries and measurement commands. The current example tasks are [171M validation loss](../tasks/171m-validation-loss.md) and [171M contact P@L](../tasks/171m-p-at-l.md), using the same small-model backbone and standard training/evaluation APIs. Both currently train two seeds for one hour each on four L40S GPUs; this is the existing example profile, not an automatic implementation of the new benchmark tracks.
...
The diagram shows the existing two-seed loop and its single-selected-recipe historical test. A benchmark submission under the new protocol instead freezes **three settings** at search completion and tests all three at equal token budgets. The loop's two-seed candidate consumes two per-seed training allowances; it must not be counted as one four-GPU-hour L40S round in the new budget table.
...
The earlier sequential campaign completed 38 candidate rounds plus a baseline, with two one-hour runs on four L40S GPUs per candidate: **78 runs and 312 L40S GPU-hours**. Each two-seed candidate therefore cost 8 GPU-hours. The retained recipes below are historical search results, not entries in the new 96-round leaderboard. Values are mean ± sample SD over seeds 42 and 43, from the [method statistics](../.dev/reports/program2/methods.tsv); the [historical example above](#historical-38-round-example) explains its acceptance rule and figure.
...
These are completed **100k-step, single-seed** comparisons, not best-of-three submissions under the new protocol. Each run used four H100s, seed 20260824, batch 1,024, context 512, base LR 5e-4, weight decay 0.01 and 1,000 warmup steps, consuming **24,200,224,761 model tokens**. Times exclude evaluation. The P@L intervals bootstrap 20,775 contact chains and do not estimate training-seed uncertainty.
...
The [78-run TSV through R38](../.dev/reports/program2/runs-through-r38.tsv), [per-method statistics through R29](../.dev/reports/program2/methods.tsv), and [original import and audit](../.dev/reports/program2/README.md) preserve the campaign. [Executed scale-up configurations](AUTORESEARCH_SCALEUP.md) and the [best-versus-baseline guide](BEST_RECIPE_VS_BASELINE.md) explain the subsequent comparison. The [detailed scale-up results](#detailed-scale-up-results) include the longer-training reference; the [overall benchmark leaderboard](autoresearch.md#overall-leaderboard) lists current submissions.
