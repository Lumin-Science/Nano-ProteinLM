...
The training clock includes batch loading and synchronization. Prepare the inputs before timing a run and keep data placement consistent across methods. Before the clock starts, the task command has the trainer read the prepared training stores into the page cache, so random batch reads during training come from memory. Record the code revision, resolved recipe, data receipts, seed, actual steps and non-padding model tokens for each run, together with its metrics and elapsed training time.
