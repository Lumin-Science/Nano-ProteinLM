...
root = Path("/scratch/muchenli/Nano-Protein-LM-nibi-setting3-b2048-100k-eval10k-20260908")
now = time.time()
gate_path = root / "QUALIFICATION_PASSED.json"
...
if (root / "QUEUE_STATUS.json").exists():
    report["queue"] = json.loads((root / "QUEUE_STATUS.json").read_text())
if (root / "QUEUE_FAILED.txt").exists():
    report["queue_failure"] = (root / "QUEUE_FAILED.txt").read_text()
for name in ("trial", "resume4", "full"):
    path = root / name
    if not path.exists():
        continue
