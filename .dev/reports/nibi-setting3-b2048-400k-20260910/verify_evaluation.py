...
NAME = "nibi-setting3-b2048-400k-20260910"
REMOTE = Path("/scratch/muchenli/Nano-Protein-LM-nibi-setting3-b2048-400k-20260910")
...
assert 110000 <= args.step <= 400000 and args.step % 10000 == 0
...
frozen_config = subprocess.check_output(
    ["git", "show", f"{SOURCE}:configs/setting3-nibi-b2048-400k.yaml"],
    cwd=REPO,
)
...
assert ready["validation_identical_to_parent"]
...
for item in (receipt, training):
    ...
    assert item["full_training_complete"] == (args.step == 400000)
...
migration = read(args.artifact_root / "DATA_MIGRATION.json")
assert contract["data_migration"] == migration
assert contract["resume_optimizer_step"] == migration["parent_step"] == 100000
assert contract["resume_checkpoint_sha256"] == migration["parent_checkpoint_sha256"]
previous = {name: sum(origin["cursors"]) for name, origin in migration["origins"].items()}
assert sum(previous.values()) == 204800000
previous_step = 100000
for r in metrics:
    assert all(
        math.isfinite(r[k])
        for k in (
    ...
    assert math.isclose(r["learning_rate"], 4.5e-4, rel_tol=1e-12)
    assert (
...
parent_root = (
    ...
    / "full/setting3/evaluations/step-100000"
)
...
comparison = {
    "reference_optimizer_steps": 100000,
